(function(){
	var updateCount = function() {
		$('#cnt'+$.cl.dictionary_id).text($('#c-dictionary :checkbox:not(:checked)').length);
	};

	/** Insert item into ul#c-dictionary */
	var insertItem = function(item, where) {

		//
		// `li.item`s in `ul#c-dictionary` are sorted by `rel` attribute.
		...
		$('#c-dictionary li.item').each(function() {
			var self = $(this);
		...
		if (insertBefore) {
			...
			$('#c-dictionary').append(li);
		}
		$('#c-dictionary').sortable('refresh');
	};
	...
	var maybeStartOver = function() {
		if ($('#c-dictionary :checkbox:not(:checked)').size() > 0 || $('#c-dictionary :checkbox').size() <= 0) {
	...
	var toggleEditMode = function(action) {
		...
		var form = $('#c-add-form');
		var cdictionary = $('#c-dictionary');
		if (action === 'off' || (action === 'toggle' && form.is(':visible'))) {
			$('#c-add-form').hide();
			cdictionary.removeClass('c-edit-mode');
		} else {
			cdictionary.addClass('c-edit-mode');
	...
	// add dictionary item when user clicks button or presses enter in input
	...
	$(':checkbox', $('#c-dictionary')[0]).die('change').live('change', function() {
		var cbox = $(this);
		var li = cbox.parents('#c-dictionary li.item');
		...
		if (cbox.is(':checked')) {
			...
			if(!li.prevAll('.c-done').length) {
				...
				if(!sort) {
					sort = $('#c-dictionary li.item').last().attr('rel');
					if (sort) {
						sort++;
					}
				}
		...
		$.post('?module=json&action=itemsave', data, function(item) {
			item = item.data;
			cbox.parents('#c-dictionary li').remove();
			insertItem(item);
			updateCount();
			maybeStartOver();
		}, 'json');
	...
	// edit items
	$('.c-edit-item', $('#c-dictionary')[0]).die('click').live('click', function() {
		if (!$.cl.can_edit) {
			return false;
		}
		var li = $(this).parents('#c-dictionary li');
	...
	// delete items
	$('.c-delete-item', $('#c-dictionary')[0]).die('click').live('click', function() {
		if (!$.cl.can_edit) {
			return false;
		}
		var li = $(this).parents('#c-dictionary li');
	...
	$('#deletedictionary').click(function() {
		...
		$.post('?module=json&action=deletedictionary', {id: $.cl.dictionary_id}, function() {
		...
		switch(e.which) {
			...
			case 40: // down
				if (current && current[0] && (!document.activeElement || current[0] === document.activeElement)) {
					...
					current = $('#c-dictionary :checkbox').first().focus();
				}
				return false;
			case 38: // up
				if (current && current[0] && (!document.activeElement || current[0] === document.activeElement)) {
					...
					current = $('#c-dictionary :checkbox').last().focus();
				}
				return false;
